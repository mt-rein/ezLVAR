...
#' @description
#' Computes the R-squared entropy index for a mixture model, which quantifies
#' the proportional reduction in classification uncertainty gained from using
#' posterior class probabilities instead of prior class proportions. A value
#' close to 1 indicates high classification certainty, whereas a value close
#' to 0 indicates substantial uncertainty in class assignments.
#'
#' @references J.K. Vermunt and J. Magidson (2016). Technical Guide for Latent
#' GOLD 5.1: Basic, Advanced, and Syntax. Belmont, MA: Statistical Innovations Inc.
#'
#' @param step3output An object obtained with the [step3] function that includes a mixture model (i.e., `mixture = TRUE`).
#'
...
r2_entropy <- function(step3output) {
  ...
  if(step3output[["type"]] != "mixture") {
    stop("The input must be the output of the step3() function including a mixture model.")
  }

  prior <- step3output$class_proportions
  posterior <- step3output$posterior_probabilities[, 2:(1+step3output$n_groups)] |>
    as.matrix()
...
#'
#' @description
#' Computes likelihood based fit indices for a Step 3 model: log-likelihood, AIC,
#'AIC3, BIC, and ICL (only for mixture models).
#'
#' @references J.K. Vermunt and J. Magidson (2016). Technical Guide for Latent
#' GOLD 5.1: Basic, Advanced, and Syntax. Belmont, MA: Statistical Innovations Inc.
#'
