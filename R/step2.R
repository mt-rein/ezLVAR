...
step2 <- function(step1output) {
  ...
  # (lists with n_blocks elements, where each element is a list with n_groups elements)
  psi_block <-
    alpha_block <-
    lambda_block <-
    theta_block <-
  ...
  # --> Lists with n_groups elements, where each element is a list with n_blocks elements
  psi_group <-
    alpha_group <-
    lambda_group <-
    theta_group <-
  ...
  for (i in 1:n_persons) {
    ...
    if (n_groups > 1) {
      ...
      if (rlang::is_empty(group_i)) {
        stop(paste0("Individual ", id_i, " does not have a valid value on the grouping variable '", group_var, "'."))
      }
    } else {
    ...
    indicators_cent <- sweep(as.matrix(data_i[, indicators]), 2, (tau_i + lambda_i %*% alpha_i), "-")
