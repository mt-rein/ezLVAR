...
#'
#' This function performs model selection using the convex hull approach as
#' described by Wilderjans et al (2013). Models with increasing complexity
#' (number of clusters) are compared based on the log likelihood or a specified
#' information criterion, and only models lying on the convex hull of the
#' fit-versus-complexity curve are retained. Scree-test values are computed for
#' the models on the hull, and the model with the highest scree-test value is
#' selected as the optimal model.
#'
#' @details
#' The [summary] function provides an overview of the CHull results, including
#' which model is selected by the procedure and the fit values of all models on
#' the hull.
#' A plot can be generated by using the [plot()] function on the output object.
#' Green points connected by a line indicate models that lie on the convex hull.
#' Red points are models above/below the convex hull. The best solution (as
#' determined by the largest scree-test value) is highlighted in blue.
#'
#'
#' @references Wilderjans, T. F., Ceulemans, E., & Meers, K. (2013). CHull: A generic convex hull based model selection method. Behavior Research Methods, 45, 1-15.
#'
...
#'   Must be one of `"logLik"` (default), `"AIC"`, `"AIC3"`,
#'   `"BIC"`, or `"ICL"`.
#'
#' @return A list with:
...
summary.CHull <-
  function(object, ...){
    ...
    solution[, c(criterion, "scree_test")] <- round(solution[, c(criterion, "scree_test")], 2)
    df_hull <- object$df_hull
    df_hull[, c(criterion, "scree_test")] <- round(df_hull[, c(criterion, "scree_test")], 2)
    cat(paste("Convex hull based on the", criterion, "criterion.\n\n"))
    if(nrow(df_hull) > 2) {
      cat("Selected model:\n")
      print(solution)
      if(nrow(df_hull) == 3) {
        cat("NOTE: There are only three models on the convex hull, so only one model was eligible for selection.\n")
      }
    } else {
      cat("No solution was selected because there are fewer than three models on the convex hull.\n")
    }
    cat("\n")
...
plot.CHull <-
  function(object, ...){
    ...
    plot(object$df_all$n_clusters, object$df_all[[object$criterion]], col = "red", pch = 16,
         xlab = "Number of clusters", ylab = object$criterion,
         xaxt = "n")
    axis(1, at = object$df_all$n_clusters, labels = object$df_all$n_clusters)
    # color points on the hull green:
    points(object$df_hull$n_clusters, object$df_hull[[object$criterion]],
           col = "green", pch = 16)
    # add green line connecting the points on the hull:
    lines(object$df_hull$n_clusters, object$df_hull[[object$criterion]], col = "green")
    # mark the solution with a blue point:
    if(nrow(object$solution) > 0) {
      points(object$solution$n_clusters, object$solution[[object$criterion]],
             col = "blue", pch = 16)
    } else {
      warning("No solution was selected because there are fewer than three models on the convex hull.")
    }
  }
